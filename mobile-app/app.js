...
function renderUsappSearchIcon() {
  return `
...
function renderUsappAttachIcon() {
  return `
    <svg class="usapp-inline-icon" viewBox="0 0 20 20" aria-hidden="true">
...
function renderThreadSettingIcon(type) {
  const icons = {
    mute: `
      <svg class="usapp-inline-icon" viewBox="0 0 20 20" aria-hidden="true">
...
function normalizeSupabaseMessage(message, contact, profilesByUserId = null) {
  ...
  const isOutgoing = Boolean(
    state.authUser
    && (
      (senderUserId && senderUserId === String(state.authUser.id || '').trim())
  ...
  const photoUrl = isOutgoing
    ? String(state.profile.photoUrl || '').trim()
    : String(
        message.photoUrl ||
        message.photo_url ||
...
function getMessageActorId() {
  return state.authUser && state.authUser.id
    ? `user-${String(state.authUser.id).trim()}`
    : String(state.actorId || '').trim();
}
...
function isCurrentActorId(actorId) {
  ...
  return value === String(state.actorId || '').trim() || value === getMessageActorId();
}
