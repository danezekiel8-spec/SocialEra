...
function normalizeSocialComment(comment) {
  return {
    ...
    photoUrl: String(comment.photoUrl || '').trim(),
...
function normalizeSocialPost(post) {
  ...
  return {
    ...
    photoUrl: String(post.photoUrl || '').trim(),
    ...
    linkedProductIds: Array.isArray(post.linkedProductIds) ? post.linkedProductIds.map((id) => String(id).trim()).filter(Boolean) : [],
