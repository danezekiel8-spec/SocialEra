...
function createSocialRoutes({
  ...
  function ensureSocialPostPersistence() {
    ...
    const missingMethod = requiredMethods.find((methodName) => (
      !socialPostPersistence || typeof socialPostPersistence[methodName] !== 'function'
    ));

    if (missingMethod) {
      const error = new Error(`Supabase social post persistence is not available. Missing ${missingMethod}.`);
      error.statusCode = 503;
      throw error;
    }

    if (
      typeof socialPostPersistence.hasWriteAccess === 'function'
      && !socialPostPersistence.hasWriteAccess()
    ) {
      const error = new Error('Supabase social post persistence is not configured for writes. Check SUPABASE_SERVICE_ROLE_KEY.');
      error.statusCode = 503;
      throw error;
    }
  }
